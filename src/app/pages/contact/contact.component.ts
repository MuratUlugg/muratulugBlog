  ...
  getValidationMessage(f:AbstractControl,name:string)
  {
    if(f.errors)
    {
      for (let errorName in f.errors) {
        ...
        else if (errorName =="email") {return `${name} Alanı Mail Formatı Doğru Değil !!!  `}
        else if (errorName =="minlength") {return `${name} Alanı En Az 10 Karakter Olmalıdır !!!  `}
      }
    }
  }
  ...
  onSubmit(){
    if(this.contactForm.valid)
    {
      ...
      this.helperService.sendContactEmail(this.contactForm.value).subscribe(data => {
        ...
        this.info = "Mesajınız tarafımıza ulaşmıştır . En kısa süre içersinde dönüş sağlayacağız.";
      },error=>{
        this.loading = false;
        this.success=false;
